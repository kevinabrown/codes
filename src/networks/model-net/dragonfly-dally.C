...
/* UGAL-G+ counters - ugal port loads for global buffers */
static long **ugal_global_port_assigned_load;
...
static int ut_broadcast_window = 5000000;
static char ut_policy[MAX_NAME_LENGTH];
static const char *global_routing_algo = "ugal_g_plus";
...
struct terminal_state
{
    ...
    float ** qos_min_token_count;    //[rail_id][qos_level] - the committed/assured rate bucket
    float ** qos_max_token_count;    //[rail_id][qos_level] - the peak/ceil rate bucket
    tw_stime ** qos_min_update_time;       //[rail_id][qos_level] - Last time the token was updated
    tw_stime ** qos_max_update_time;       //[rail_id][qos_level] - Last time the token was updated

    int* last_qos_lvl; //[rail_id]
    int** last_qos_lvl_p; //[rail_id][qos_level]
...
struct router_state
{
    ...
    int** route_upper_threshold_for_min_score;    // counts when upper threshold is exceeded while min score is best
    int** route_upper_threshold_for_nonmin_score; // counts when upper threshold is exceeded while nonmin score is best
...
static inline int dfdally_apply_advanced_scoring(router_state *s, tw_bf *bf, terminal_dally_message *msg, int port, tw_lp *lp, conn_minimality_t c_minimality, int score, bool is_global_adaptive, int group_code)
{
    /* Do nothing if we're doing DELTA scoring. DELTA has a pre-applied
     * 2:1 minimal bias */
    if (port == -1 || scoring == DELTA)
    {
        return score;
    }
    ...
    if (is_global_adaptive && scoring_factors != NULL)
    { // this is a global adaptive decision and inter-group scoring factors are set
...
/* Checks in non-source group*/
static inline bool exceed_adaptive_upper_threshold_general(router_state *s, tw_bf *bf, terminal_dally_message *msg, Connection conn, conn_minimality_t c_minimality)
{ // TODO add validation checks

    // Does not need to recieve the dest_group as a param since we should only call this function in the intm_group or dest_group.
    ...
    if(s->voq_occupancy_total[port] > s->params->adaptive_threshold_upper)
        return true;
 ...
 *     threshold as well.
 *     */
static inline bool exceed_adaptive_upper_threshold(router_state *s, tw_bf *bf, terminal_dally_message *msg, Connection conn, conn_minimality_t c_minimality, int fdest_group_id)
{
    int port = conn.port;
    if (port == -1 || s->params->adaptive_threshold_upper == -1) {
        return false;
    ...
    int src_group_id = msg->origin_router_id / s->params->num_routers;
    
    // This function should only be called in the source group
    assert(my_group_id == src_group_id);
    ...
    if(conn.conn_type == CONN_GLOBAL){
        ...
        // Check if the downstream router is congested
...
static Connection get_absolute_best_connection_from_conn_set(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp, set< Connection > conns)
{
    ...
    if (conns.size() == 1) { //no need to compare singular connection
        #if DEBUG_ROUTING_SCORE == 1
        if(SCORED_ROUTER(s->router_id)) {
            fprintf(dragonfly_route_score_log, "%d:%d", (conns.begin())->port, -1);
        }
        #endif
...
void dragonfly_print_params(const dragonfly_param *p, FILE * st)
{
    ...
    char tmp_str[20], priorities[20] = "", min_bandwidth[60] = "", max_bandwidth[60] = "";
    ...
    {
        sprintf(tmp_str, " %3d |", p->qos_priorities[i]);
        strcat(priorities, tmp_str);
...
static void dragonfly_read_config(const char * anno, dragonfly_param *params)
{
    ...
    p->qos_priorities = (int*)calloc(p->num_qos_levels, sizeof(int));
    char qos_priorities_str[MAX_NAME_LENGTH];
    rc = configuration_get_value(&config, "PARAMS", "qos_priorities", anno, qos_priorities_str, MAX_NAME_LENGTH);
    if(rc) {
	int i;
        for(i = 0; i < p->num_qos_levels; i++)
	        p->qos_priorities[i] = i;
    }
    ...
    if(p->num_qos_levels > 1)
    {
        ...
        token = strtok(qos_priorities_str, ",");
        int i = 0;
        while(token != NULL)
        {
            sscanf(token, "%d", &p->qos_priorities[i]);
            assert(p->qos_priorities[i] < p->num_qos_levels);
            i++;
    ...
    rc = configuration_get_value_int(&config, "PARAMS", "ut_broadcast_window", anno, &ut_broadcast_window); // KBEDIT
    if(rc) {
        if(!myRank)
            fprintf(stderr, "Setting ut_broadcast_window to %d\n", ut_broadcast_window);
	}
    configuration_get_value(&config, "PARAMS", "ut_policy", anno, ut_policy,
            MAX_NAME_LENGTH);
...
static void update_accumulated_tokens(tw_stime now, terminal_state * s, int rail_id, int qos_lvl)
{

    if (s->qos_min_token_count[rail_id][qos_lvl] == qos_bucket_max && s->qos_max_token_count[rail_id][qos_lvl] == qos_bucket_max)
    {
    ...
    tw_stime min_elapsed_time = now - s->qos_min_update_time[rail_id][qos_lvl];
    tw_stime max_elapsed_time = now - s->qos_max_update_time[rail_id][qos_lvl];
    ...
    if(min_accum_tokens >= 1.0f)
    {
        ...
        if(s->qos_min_token_count[rail_id][qos_lvl] + whole_tokens >= qos_bucket_max)
        {
            s->qos_min_token_count[rail_id][qos_lvl] = qos_bucket_max; // Any truncation?
        }else
        {
            s->qos_min_token_count[rail_id][qos_lvl] += whole_tokens;
        }
        assert(s->qos_min_token_count[rail_id][qos_lvl] >= 0.0 && s->qos_min_token_count[rail_id][qos_lvl] <= qos_bucket_max);
        ...
        s->qos_min_update_time[rail_id][qos_lvl] = now - part_token_time; 
    }
    if(max_accum_tokens >= 1.0f)
    {
        ...
        if(s->qos_max_token_count[rail_id][qos_lvl] + whole_tokens >= qos_bucket_max)
        {
            s->qos_max_token_count[rail_id][qos_lvl] = qos_bucket_max; // Any truncation?
        }else
        {
            s->qos_max_token_count[rail_id][qos_lvl] += whole_tokens;
        }
        assert(s->qos_max_token_count[rail_id][qos_lvl] >= 0.0);
        assert(s->qos_max_token_count[rail_id][qos_lvl] <= qos_bucket_max);

        // Record updated time as time for last whole token /
        double part_token_time = (part_token * s->params->chunk_size) / qos_max_bytes_per_ns;
        s->qos_max_update_time[rail_id][qos_lvl] = now - part_token_time; 
    }

    return;
}

static double get_term_bandwidth_consumption(terminal_state * s, int rail_id, int qos_lvl)
{
...
//    int percent_bw = (bw_consumed / s->params->cn_bandwidth) * 100;
    double percent_bw = (((double)s->qos_data[rail_id][qos_lvl]) / max_bytes_per_win) * 100;
...
void issue_rtr_bw_monitor_event(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp)
{
    ...
    if(dragonfly_rtr_rtg_log != NULL){
        for(int i = 0; i < radix; i++)
            for(int j = 0; j < num_qos_levels; j++){
                if(s->route_min_score[i][j] > 0 || s->route_lower_threshold[i][j] > 0 || s->route_upper_threshold_for_min_score[i][j] > 0 || s->route_upper_threshold_for_nonmin_score[i][j] > 0 || s->route_nonmin_score[i][j] > 0){
                    fprintf(dragonfly_rtr_rtg_log, "\n %.0f %d %d %d %d %d %d %d %d %d", tw_now(lp), s->router_id, i, j, s->route_min_score[i][j], s->route_min_score_only[i][j], s->route_lower_threshold[i][j], s->route_upper_threshold_for_min_score[i][j], s->route_upper_threshold_for_nonmin_score[i][j], s->route_nonmin_score[i][j]);

                    // This is repeated for each qos level for a given switch. It should be elsewhere //KBEDIT
                    fprintf(dragonfly_rtr_rtg_log, " %d", s->group_link_update_count);
                    s->group_link_update_count = 0;

                    s->route_min_score[i][j] = 0;
                    s->route_min_score_only[i][j] = 0;
                    s->route_lower_threshold[i][j] = 0;
                    s->route_upper_threshold_for_min_score[i][j] = 0;
                    s->route_upper_threshold_for_nonmin_score[i][j] = 0;
                    s->route_nonmin_score[i][j] = 0;
                }
            }
    }
    #endif
...
//    int bw_consumption[num_qos_levels];
    ...
    {
        // = (int *) calloc(num_qos_levels, sizeof(int));   // to remove
        ...
        if(first_green >= 0)
        {
            s->qos_min_token_count[rail_id][first_green] -= 1.0f;
            if(s->qos_max_token_count[rail_id][first_green] >= 1.0f)
                s->qos_max_token_count[rail_id][first_green] -= 1.0f;

            assert(s->qos_min_token_count[rail_id][first_green] >= 0.0);
            assert(s->qos_max_token_count[rail_id][first_green] >= 0.0);

            return first_green;
        }
        else if(first_yellow >= 0)
        {
            s->qos_max_token_count[rail_id][first_yellow] -= 1.0f;
            
            assert(s->qos_max_token_count[rail_id][first_yellow] >= 0.0);
    ...
    int next_rr_vcg = (s->last_qos_lvl[rail_id] + 1) % num_qos_levels;
...
static int get_next_vcg(terminal_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{
    ...
    if(BW_MONITOR == 1)
    {
        for(int k = 0; k < num_qos_levels; k++)
        {
            if(s->qos_status[msg->rail_id][k] != Q_INACTIVE)
            {
        ...
        * priority traffic first. */
        ...
        for(int i = 0; i < num_qos_levels; i++)
        {
            if(s->qos_status[msg->rail_id][i] == Q_ACTIVE_UNSATED)
            {
        ...
        for(int i = 0; i < num_qos_levels; i++)
        {
            if(s->qos_status[msg->rail_id][i] == Q_ACTIVE_SATED)
            {
                if(s->terminal_msgs[msg->rail_id][i] != NULL && s->vc_occupancy[msg->rail_id][i] + s->params->chunk_size <= s->params->cn_vc_size)
                    return i;
            }
...
static inline int get_end_of_vcg(int curr_vcg, int vcs_per_qos)
{
    return (curr_vcg * vcs_per_qos) + vcs_per_qos - 1;
}
...
//    int bw_consumption[num_qos_levels];
    ...
    if(BW_MONITOR == 1 && num_qos_levels > 1)
    {
        int first_green = -1;       // Marks the VC that can send next
        int first_yellow = -1;      // If no classes are marked green, this VC will send next
        int *vcg_marked_status = (int*)calloc(num_qos_levels, sizeof(int));

        // Update token buckets with newly accumulated tokens
        for(int j = 0; j < num_qos_levels; j++)
        {
            update_rtr_accumulated_tokens(tw_now(lp), s, j, output_port);
            #if DEBUG_QOS_X == 1
            printf("[%.0lf] qos_token_accumulate router:%d port:%d class:%d min_tokens:%.2f max_tokens:%.2f\n", 
                    tw_now(lp), s->router_id, output_port, j,
                    s->qos_min_token_count[output_port][j],
                    s->qos_max_token_count[output_port][j]);
            #endif
        }

        // Loop over all priority levels //
        for(int curr_priority = 0; curr_priority < num_qos_levels; curr_priority++)
        {
            int start_vc = (s->last_qos_lvl_p[output_port][curr_priority] + 1) % s->params->num_vcs;
            
            /*// These flags are used per VCG. The are reset every time we change VCG or priority
            int green = false;
            int yellow = false;
            int red = false;
            */

             // Loop over all vc starting with next VC in line for this priority level - Prevent markingt the same VC twice at this priority level//
            for (int i = start_vc; i < start_vc + s->params->num_vcs; i++){
                
                int curr_vc = i % s->params->num_vcs;

                // Skip this VCG if it is not on the current priorty level or if it has already been marked//
                int curr_vcg = curr_vc / vcs_per_qos;
                if (s->params->qos_priorities[curr_vcg] != curr_priority || vcg_marked_status[curr_vcg] == 1){
                    // This can be optimized by jumping over all VCs in this VCG
                    // i = i + (vc_per_qos-1)?
                    continue;
                }
            ...
            //{
                ...
                if(s->pending_msgs[output_port][curr_vc] != NULL)
                {
                    // Check if we have enough downstream credit for this chunck
                    if(router_downstream_credit_available(s, output_port, curr_vc, chunk_size)) // KBEdit: This will block sending for chunks small than chunk_size in some cases
                    {
                        /* Check if this is a red class: it has insufficient tokens */
                        if(s->qos_max_token_count[output_port][curr_vcg] < 1.0f)
                        { // RED class

                            //red = true;
                            i = get_end_of_vcg(curr_vcg, vcs_per_qos);

                            #if DEBUG_QOS == 1
                            s->qos_red_total[output_port][curr_vcg]++;
                            #endif
                            vcg_marked_status[curr_vcg] = 1;
                            continue;
                        }
                        /* Check if this is a  yellow class: it has insufficient assured rate tokens */
                        else if(s->qos_min_token_count[output_port][curr_vcg] < 1.0f)
                        { // YELLOW CLASS

                            //yellow = true;
                            if(first_yellow < 0)
                            {
                                first_yellow = curr_vc;
                            }
                            i = get_end_of_vcg(curr_vcg, vcs_per_qos);

                            #if DEBUG_QOS == 1
                            s->qos_yellow_total[output_port][curr_vcg]++;
                            #endif
                            vcg_marked_status[curr_vcg] = 1;
                            continue;
                        }
                        /* If the class is neither red nor yellow, it is red */
                        else
                        { // GREEN CLASS
                            ...
                            #if DEBUG_QOS == 1
                            s->qos_green_total[output_port][curr_vcg]++;
                            #endif
                            vcg_marked_status[curr_vcg] = 1;
                            continue;
                        }
...
void terminal_dally_init( terminal_state * s, tw_lp * lp )
{
    ...
    if(s->terminal_id == 0)
    {
        ...
        fprintf(rdfdally_file, "time destination source intm.grp ugal.intm.grp qosclass num.g.hops num.l.hops nm.sgrp nm.igrp nm.dgrp latency router.queue.time");
#endif
    }
    #if DEBUG_QOS_T == 1
    if(s->terminal_id == 0)
    ...
    s->qos_min_token_count = (float**)calloc(p->num_rails, sizeof(float*));
    s->qos_max_token_count = (float**)calloc(p->num_rails, sizeof(float*));
    s->qos_min_update_time = (tw_stime**)calloc(p->num_rails, sizeof(tw_stime*));
    s->qos_max_update_time = (tw_stime**)calloc(p->num_rails, sizeof(tw_stime*));

    s->last_qos_lvl = (int*)calloc(p->num_rails, sizeof(int));
    s->last_qos_lvl_p = (int**)calloc(p->num_rails, sizeof(int*));
    ...
    for(i = 0; i < p->num_rails; i++)
    {
        ...
        s->qos_min_token_count[i] = (float*)calloc(num_qos_levels, sizeof(float));
        s->qos_max_token_count[i] = (float*)calloc(num_qos_levels, sizeof(float));
        s->qos_min_update_time[i] = (tw_stime*)calloc(num_qos_levels, sizeof(tw_stime));
        s->qos_max_update_time[i] = (tw_stime*)calloc(num_qos_levels, sizeof(tw_stime));

        s->last_qos_lvl_p[i] = (int*)calloc(num_qos_levels, sizeof(int));  // KBEdit - it should be ok to not explicitly initialize when using calloc, right?
        ...
        for(int j = 0; j < num_qos_levels; j++)
        {
            ...
            s->qos_min_token_count[i][j] = 0;
            s->qos_max_token_count[i][j] = 0;
            s->qos_min_update_time[i][j] = 0;
            s->qos_max_update_time[i][j] = 0;
        }
    }
...
void router_dally_init(router_state * r, tw_lp * lp)
{
    ...
    if(dragonfly_rtr_rtg_log == NULL)
    {
        ...
        fprintf(dragonfly_rtr_rtg_log, "\n time-stamp router-id port-id qos-level by-min-score by-min-score-only by-lower-threshold by-upper-threshold-for-min-score by-upper-threshold-for-nonmin-score by-nonmin-score group-link-updates");
    }
    #endif
    #if DEBUG_ROUTING_SCORE == 1
    ...
    if(dragonfly_route_score_log == NULL)
    {
        ...
        fprintf(dragonfly_route_score_log, "\n time-stamp router-id qos-level location path-type port-scores");
    }
    #endif
   ...
   //printf("\n Local router id %d global id %d ", r->router_id, lp->gid);
    ...
    r->voq_occupancy_total = (int*)calloc(p->radix, sizeof(int));
    if (r->router_id == 0){ /* This does not work in parallel mode */
        ugal_global_port_assigned_load = (long**)calloc(p->num_groups, sizeof(long*) * p->num_groups);
        for(int i = 0; i < p->num_groups; i++)
            ugal_global_port_assigned_load[i] = (long*)calloc(p->num_groups, sizeof(long) * p->num_groups);
    }
    r->group_link_my_ids = (int*)calloc(p->num_global_channels, sizeof(int));
    ...
    r->group_buffer_occupancy = (int*)calloc(p->num_groups, sizeof(int));
    ...
    r->last_qos_lvl_p = (int**)calloc(p->radix, sizeof(int*));
...
#if DEBUG_ROUTING_DECISION == 1
    r->route_min_score = (int**)calloc(p->radix, sizeof(int*));
    r->route_min_score_only = (int**)calloc(p->radix, sizeof(int*));
    r->route_lower_threshold = (int**)calloc(p->radix, sizeof(int*));
    r->route_upper_threshold_for_min_score = (int**)calloc(p->radix, sizeof(int*));
    r->route_upper_threshold_for_nonmin_score = (int**)calloc(p->radix, sizeof(int*));
    r->route_nonmin_score = (int**)calloc(p->radix, sizeof(int*));
    for(int i=0; i < p->radix; i++)
    {
        r->route_min_score[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->route_min_score_only[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->route_lower_threshold[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->route_upper_threshold_for_min_score[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->route_upper_threshold_for_nonmin_score[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->route_nonmin_score[i] = (int*)calloc(num_qos_levels, sizeof(int));
        for(int j = 0; j < num_qos_levels; j++){
            r->route_min_score[i][j] = 0;
            r->route_min_score_only[i][j] = 0;
            r->route_lower_threshold[i][j] = 0;
            r->route_upper_threshold_for_min_score[i][j] = 0;
            r->route_upper_threshold_for_nonmin_score[i][j] = 0;
            r->route_nonmin_score[i][j] = 0;
        }
    }
#endif
    ...
    for(int i=0; i < p->radix; i++)
    {
       // Set credit & router occupancy
        ...
        r->last_qos_lvl[i] = 0;
        r->last_qos_lvl_p[i] = (int*)calloc(num_qos_levels, sizeof(int));
        r->link_traffic[i]=0;
    ...
    if(ut_policy != NULL){ // populate my IDs
        vector< Connection > my_global_links = r->connMan.get_connections_by_type(CONN_GLOBAL);
        vector< Connection >::iterator itg = my_global_links.begin();
        int i=0;
        if(r->router_id == 1  && tw_now(lp) < 500){
            printf("\n---------- Setup group IDS on router: %d", r->router_id);
        }
        for(; itg != my_global_links.end(); itg++)
        {
            // Get router's gid
            //int id = itg->dest_gid;
            //int dest_group_id =  id / p->num_routers;
            r->group_link_my_ids[i] = itg->dest_group_id;
            i++;
        }
    }
    //if(strstr(ut_policy, "window") != NULL)
    if(strcmp(ut_policy, "window") == 0)
    {
        tw_stime bcast_ts = ut_broadcast_window;
        terminal_dally_message * m3;
        tw_event * e3 = model_net_method_event_new(lp->gid, bcast_ts, lp,
            DRAGONFLY_DALLY_ROUTER, (void**)&m3, NULL);
        m3->type = R_GROUP_BROADCAST;
        m3->magic = router_magic_num;
        tw_event_send(e3);
    }
...
static void packet_send_rc(terminal_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{
    ...
    if(msg->qos_reset1)
        s->qos_status[msg->rail_id][0] = Q_ACTIVE_UNSATED;
    if(msg->qos_reset2)
        s->qos_status[msg->rail_id][1] = Q_ACTIVE_UNSATED;
...
static void packet_send(terminal_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp) 
{
    ...
    * of VCs*/
    ...
    m->travel_start_time = s->terminal_available_time[msg->rail_id] - injection_ts;
    m->type = R_ARRIVE;
    ...
    m->is_intm_visited = 0;
    m->intm_grp_id = -1;
    m->ugal_intm_grp_id = -1;
    m->ugalp_intm_grp_id = -1;
...
static void packet_arrive_rc(terminal_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{
    ...
    if(bf->c21)
        s->min_latency[vcg] = msg->saved_min_lat;
    if(bf->c22)
	{
...
static void packet_arrive(terminal_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp) 
{
    ...
    s->total_time[vcg] += ete_latency; 
    s->period_total_time[vcg] += ete_latency; 
    total_hops[vcg] += msg->my_N_hop;
    s->total_hops[vcg] += msg->my_N_hop;
    s->fin_hops_sample += msg->my_N_hop;
    s->ross_sample.fin_hops_sample += msg->my_N_hop;
    ...
    fprintf(rdfdally_file, "\n%lf %d %d %d %d %d %d %d %d %hi %hi %hi %lf %lf", tw_now(lp), s->terminal_id,
            codes_mapping_get_lp_relative_id(msg->sender_mn_lp,0,0), msg->intm_grp_id, msg->ugal_intm_grp_id, msg->ugalp_intm_grp_id,
    ...
    if(s->min_latency[vcg] > ete_latency) {
        bf->c21 = 1;
        msg->saved_min_lat = s->min_latency[vcg];
		s->min_latency[vcg] = ete_latency;	
	}
    if(s->period_min_latency[vcg] > tw_now(lp) - msg->travel_start_time) {
		s->period_min_latency[vcg] = tw_now(lp) - msg->travel_start_time;	
	}

	if(s->max_latency[vcg] < ete_latency) {
        bf->c22 = 1;
        msg->saved_available_time = s->max_latency[vcg];
        s->max_latency[vcg] = ete_latency;
	}
...
//        fprintf(fp, "# Format <LP id> <Terminal ID> <Total Data Size> <Avg packet latency> <# Flits/Packets finished> <Avg hops> <Busy Time> <Max packet Latency> <Min packet Latency >\n");
    ...
    if(s->terminal_id == 0)
    {
        written += sprintf(s->output_buf2 + written, "# Format <LP id> <Terminal ID> <QoS class> <Total Data Sent> <Total Data Received> <Avg packet latency> <Max packet Latency> <Min packet Latency> <# Packets finished> <Avg Hops> <Avg Busy Time (over rails)>\n");
    }
    ...
    for(int i = 0; i < dragonfly_num_qos_levels; i++)
    {
        written += sprintf(s->output_buf2 + written, "%llu %u %d %ld %llu %lf %lf %lf %ld %lf %lf\n", 
                ...
                s->finished_packets[i], (double)s->total_hops[i]/s->finished_chunks[i], avg_busy_time);
    }
    ...
    if (!isRoutingSmart(routing))
    {
        ...
        if (routing != PROG_ADAPTIVE_LEGACY && routing != PROG_ADAPTIVE)
        {
        	...
        	if(SCORED_ROUTER(s->router_id)) {
                    ...
                    fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d ", tw_now(lp), s->router_id, get_vcg_from_category(msg), 2, 4);
                }
                #endif
        	    ...
        	    if(SCORED_ROUTER(s->router_id)) {
                        // get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
                        fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d ", tw_now(lp), s->router_id, get_vcg_from_category(msg), 2, 5); // 0 for min connection type
                    }
                    #endif
            ...
            if(msg->last_hop == TERMINAL) //This is used by dfdally_nonminimal_routing
            {
...
// Broadcast the state of global channel loads to other routers in group - this could be combined with the function above
static void router_buf_state_send(router_state * s, terminal_dally_message * msg, 
  tw_lp * lp, int local_id, tw_lpid dst_lpid) {
    tw_event * buf_e;
    terminal_dally_message * buf_msg;

    const dragonfly_param *p = s->params;

    double broadcast_delay = 5;  // unused now, but may be used in the future. This can allow us to vary the time we broadcast updates

    buf_e = model_net_method_event_new(dst_lpid, broadcast_delay, lp, DRAGONFLY_DALLY_ROUTER,
            (void**)&buf_msg, NULL);
    ...
    for(int j = 0; j < p->num_global_channels; j++){ // Currently using an awkward layout: 1D array of group_ids followed by link states
        dest_group_id = s->group_link_my_ids[j];
        assert(dest_group_id >= 0 && dest_group_id < p->num_groups);

        buf_msg->rc_qos_status[j] = dest_group_id;
        buf_msg->rc_qos_status[j + p->num_global_channels] = s->group_link_status[dest_group_id]; //s->voq_occupancy_total[j];
    ...
    buf_msg->magic = router_magic_num;
    buf_msg->saved_sample_time = tw_now(lp);
    buf_msg->origin_router_id = local_id;
    buf_msg->type = R_GROUP_BUFFER;

    tw_event_send(buf_e);
    return;
}

static void router_buf_notify_update(router_state * s, terminal_dally_message * msg, 
  tw_lp * lp, int dest_group_id) {
    
    const dragonfly_param *p = s->params;
    double broadcast_delay = 2;  // unused now, but may be used in the future. This can allow us to vary the time we broadcast updates

    //if(port >= 0 && port < p->radix){ // if we are supplied with a valid port
    //    if(port < p->intra_grp_radix || port >= p->intra_grp_radix +  p->num_global_channels) // if port is not global, do nothing if the udpated port is not global
    //        return;
    //} else
    //    assert(port == -1); // The only acceptable non-valid port in this function is -1, which indicates a port was not supplied
    ...
    for(; itl != my_local_links.end(); itl++)
    {
        ...
        buf_e = model_net_method_event_new(router_dest_id, broadcast_delay, lp, DRAGONFLY_DALLY_ROUTER,
                (void**)&buf_msg, NULL);

        // Complete preparing and sending event
        buf_msg->magic = router_magic_num;
        buf_msg->total_size = s->group_buffer_occupancy[dest_group_id];
        buf_msg->vc_index = dest_group_id;
        buf_msg->type = R_GROUP_BUFFER;
...
//TODO: need to find a more efficient way to write this function without all the lookups. Also, don't check/update all ports when only a single port occupancy changed -kevinB
static void router_buf_notify_update_state(router_state * s, terminal_dally_message * msg, 
  tw_lp * lp, int port) {
    
    const dragonfly_param *p = s->params;

    if(p->adaptive_threshold_upper == -1) // Do nothing if the adaptive threshold is unset
        return;
    ...
    tw_stime min_update_window = 2; // Minimum time between make/posting status updates
        // If it has been longer than min_update_window since the last update was posted, post update now
        //if(tw_now(lp) - s->group_link_status_time[my_id] > min_update_window){
    
    // debug-ut
    //if(s->router_id == 1 && tw_now(lp) < 500){
    //    printf("\n -=-=-=-=-[%d(L: %d)] Broadcasting updates for groups |",s->router_id, my_local_id);
    //}
    ...
    for(; itg != my_global_links.end(); itg++)
    {
        ...
        int id = itg->dest_gid;
        port = itg->port;
        dest_group_id = itg->dest_group_id; //id / s->params->num_routers;
        
        // Check if global links went above or below the threshold based on last record for the link
        //  state of '-1': link is at or below threshold
        //  state of '1' : link is above threhsold
        if(s->voq_occupancy_total[port] > p->adaptive_threshold_upper){
            if(s->group_link_status[dest_group_id] == -1){
                s->group_link_status[dest_group_id] = 1;
                s->group_link_status_time[dest_group_id] = tw_now(lp);
                s->group_link_update_count++;
    ...
    if(status_changed){ // If any port status has changed, broadcast updated
        vector< Connection > my_local_links = s->connMan.get_connections_by_type(CONN_LOCAL);
        vector< Connection >::iterator itl = my_local_links.begin();
        for(; itl != my_local_links.end(); itl++)
        {
...
//        max_vc_size = s->params->global_vc_size;
    ...
    /* KBEdit update ugal_g channel loads as necessary */
    if(strcmp(global_routing_algo, "ugal_g_plus") == 0 && my_group_id != dest_group_id) 
    { // We are not in the destination group, so a global hop will be necessary
        if(cur_chunk->msg.last_hop == TERMINAL || cur_chunk->msg.last_hop == GLOBAL)
        { // We are on the first router in this group
            output_chan = 0;
        }
        else if (cur_chunk->msg.last_hop == GLOBAL)
        { // We just entered a new group. Increment the VC.
    ...
    /*if (
    ugal_global_port_assigned_load[s->group_id][potential_intm_group];
    if (my_group_id == src_group_id)
    {
        output_chan = cur_chunk->msg.my_l_hop;
        if(msg->path_type == NON_MINIMAL)
    ...
    if(strcmp(ut_policy, "instant") == 0){
        router_buf_notify_update_state(s, msg, lp, output_port); // TODO: maybe msg is not the right paramter
    }
    if(strcmp(global_routing_algo, "ugal_g_plus") == 0) {
        s->group_buffer_occupancy[next_stop_conn.dest_group_id] = s->voq_occupancy_total[output_port];
        router_buf_notify_update(s, msg, lp, next_stop_conn.dest_group_id); // TODO: maybe msg is not the right paramter
    }
...
static void router_packet_send( router_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{
    ...
    * next.*/
    ...
    if(strcmp(ut_policy, "instant") == 0){
        router_buf_notify_update_state(s, msg, lp, output_port); // TODO: maybe msg is not the correct thing to send
    }
    /*if(strcmp(global_routing_algo, "ugal_g_plus") == 0) {
        Connection prev_conn = s->connMan.get_connection_on_port(output_port);
        s->group_buffer_occupancy[prev_conn.dest_group_id] = s->voq_occupancy_total[output_port];
        router_buf_notify_update(s, msg, lp, prev_conn.dest_group_id); // TODO: maybe msg is not the right paramter
    }*/
...
static void router_group_buf_update_rc(router_state * s,
        tw_bf * bf,
        terminal_dally_message * msg,
        tw_lp * lp)
{
...
static void router_group_buf_update(router_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{// TODO: Add RC
 
    const dragonfly_param *p = s->params;

    int group_id = msg->vc_index;
    assert(group_id >= 0 && group_id < p->num_groups);
    s->group_buffer_occupancy[group_id] = msg->total_size;

    // Record time of update
    //s->group_link_status_time[group_id] = msg->saved_sample_time;
    
    // debug-ut
    //if(s->router_id < 16 && s->router_id != 1  && tw_now(lp) < 500){
    //    printf("\n ========== [%d] Recieving updates from local ID: %d for groups |", s->router_id, local_id);
    //}
    // Copy updated states
    //for(int j = 0; j < p->num_global_channels; j++) {
        //s->group_link_status[dest_group_id] = msg->rc_qos_status[j + p->num_global_channels];
...
static void router_group_buf_update_state_rc(router_state * s,
        tw_bf * bf,
        terminal_dally_message * msg,
        tw_lp * lp)
{
...
static void router_group_buf_update_state(router_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{// TODO: Add RC
 
    const dragonfly_param *p = s->params;

    // Get source router offset within group
    int local_id = msg->origin_router_id;
    int group_id = msg->vc_index;
    ...
    int dest_group_id;
    for(int j = 0; j < p->num_global_channels; j++) { // Currently using an awkward layout: 1D array of group_ids followed by link states
        dest_group_id = msg->rc_qos_status[j];
        assert(dest_group_id >= 0 && dest_group_id < p->num_groups);
        s->group_link_status[dest_group_id] = msg->rc_qos_status[j + p->num_global_channels];
...
static void router_group_broadcast_rc(router_state * s,
        tw_bf * bf,
        terminal_dally_message * msg,
        tw_lp * lp)
{
...
static void router_group_broadcast(router_state * s, tw_bf * bf, terminal_dally_message * msg, tw_lp * lp)
{// TODO: Add RC
 

    router_buf_notify_update_state(s, msg, lp, -1);
    // Get source router offset within group
    if(strcmp(ut_policy, "window") == 0)
    {
        tw_stime bcast_ts = ut_broadcast_window;
    ...
    tw_lp * lp) 
{
    ...
    // TO CLEAR BY KEVIN - used for debugging UT
    //if(tw_now(lp) > 75000.0 && msg->type != R_BANDWIDTH)
    //    return;
    ...
    switch(msg->type)
    {
        ...
        case R_GROUP_BUFFER:
            //printf("\n =-=-=-=-=-=- %d: group router buf update\n", s->router_id);
            router_group_buf_update(s, bf, msg, lp);
        break;

        case R_GROUP_STATE:
            //printf("\n =-=-=-=-=-=- %d: group router state update\n", s->router_id);
            router_group_buf_update_state(s, bf, msg, lp);
        break;

        case R_GROUP_BROADCAST:
            printf("\n =-=-=-=-=-=- %d: group router buf broadcast\n", s->router_id);
            router_group_broadcast(s, bf, msg, lp);
        break;
  ...
  terminal_dally_message * msg, tw_lp * lp) 
{
    ...
    switch(msg->type) {
        ...
        case R_GROUP_BUFFER:
            router_group_buf_update_rc(s, bf, msg, lp);
        break;
        
        case R_GROUP_STATE:
            router_group_buf_update_state_rc(s, bf, msg, lp);
        break;
        
        case R_GROUP_BROADCAST:
            router_group_broadcast_rc(s, bf, msg, lp);
        break;
...
// Find best exit router in the current group based on ugal-g
static Connection ugal_get_best_conn_to_exit_router(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp, vector< int > group_list)
{
    ...
    int num_to_compare = group_list.size();
    int scores[num_to_compare]; // = {90, 80, 70, 60, 50, 40, 30};
    vector < int > best_groups;
    vector < Connection > best_conns, temp_conns;
    int best_score = INT_MAX, group = -1;

    //if(s->router_id == 5) tw_output(lp,"\n Global port scores: "); //KBDEBUG
    for(int i = 0; i < num_to_compare; i++){
        group = group_list[i];
        //if(s->router_id == 5) tw_output(lp,"\n | group: %d", group); //KBDEBUG

        scores[i] = s->group_buffer_occupancy[group];
        //if(s->router_id == 5) tw_output(lp," | score: %d", scores[i]); //KBDEBUG

        if (scores[i] <= best_score) {
            if (scores[i] < best_score) {
                best_score = scores[i];
                best_groups.clear();
                best_groups.push_back(group);
            }
            else {
                best_groups.push_back(group);
            }
    ...
    //if(s->router_id == 5) tw_output(lp,"\n Best groups: "); //KBDEBUG
    vector < int > best_groups_full;
    for(int i = 0 ; i < best_groups.size() ; i++){
        //if(s->router_id == 5) tw_output(lp,"\n | group: %d", best_groups[i]); //KBDEBUG

        // no direct links exist here, so get indirect links
        temp_conns = s->connMan.get_routed_connections_to_group(best_groups[i], true);
        if(temp_conns.size() > 0){
            for(int j = 0; j < temp_conns.size(); j++){
                //if(s->router_id == 5) tw_output(lp,"\n |+conn.lid: %d, conn.gid: %d", temp_conns[j].dest_lid, temp_conns[j].dest_gid); //KBDEBUG
                best_conns.push_back(temp_conns[j]);
                best_groups_full.push_back(best_groups[i]);
            }
        }
        else {
            temp_conns = s->connMan.get_connections_to_group(best_groups[i]);
            if(temp_conns.size() > 0){
                for(int j = 0; j < temp_conns.size(); j++){
                    //if(s->router_id == 5) tw_output(lp,"\n |-conn.lid: %d, conn.gid: %d", temp_conns[j].dest_lid, temp_conns[j].dest_gid); //KBDEBUG
                    best_conns.push_back(temp_conns[j]);
    ...
    msg->num_rngs++;
    int rand = tw_rand_integer(lp->rng, 0, best_conns.size()-1);
    msg->ugal_intm_grp_id = best_groups_full[rand];
    return best_conns[rand];
}

// Find best exit router in the current group based on ugal-g-plus
static Connection ugalp_get_best_conn_to_exit_router(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp, vector< int > group_list)
{
    if (group_list.size() == 0) {
        Connection bad_conn;
        bad_conn.src_gid = -1;
    ...
    vector < int > best_groups;
    vector < Connection > best_conns, temp_conns;
    int best_score = INT_MAX, potential_intm_group = -1;

    //if(s->router_id == 5) tw_output(lp,"\n Global port scores: "); //KBDEBUG
    for(int i = 0; i < num_to_compare; i++){
        potential_intm_group = group_list[i];
        //if(s->router_id == 5) tw_output(lp,"\n | group: %d", group); //KBDEBUG

        scores[i] = ugal_global_port_assigned_load[s->group_id][potential_intm_group];
        //if(s->router_id == 5) tw_output(lp," | score: %d", scores[i]); //KBDEBUG
        ...
        if (scores[i] <= best_score) {
            if (scores[i] < best_score) {
                best_score = scores[i];
                best_groups.clear();
                best_groups.push_back(potential_intm_group);
            }
            else {
                best_groups.push_back(potential_intm_group);
            }
    ...
    msg->num_rngs++;
    int rand = tw_rand_integer(lp->rng, 0, best_conns.size()-1);
    msg->ugalp_intm_grp_id = best_groups_full[rand];
    return best_conns[rand];
}
...
static vector< Connection > get_legal_nonminimal_stops_new(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp, int fdest_router_id)
{
    ...
    if (my_group_id == origin_group_id) { // We are in origin group. Local nonmin is not allowed, only global nonmin
        if (my_router_id == msg->origin_router_id) { //then we are able to route within our own group if necessary
            ...
            //if(my_router_id == 5 && msg->packet_ID >= 201 && msg->packet_ID < 205 && msg->src_terminal_id == 27)
            if(strcmp(global_routing_algo, "ugal_g_plus") == 0 && my_group_id != fdest_group_id) {
                vector < Connection > conns;
                //tw_output(lp,"\nPacketID: %d", msg->packet_ID); //KBDEBUG
                Connection best_conn = ugal_get_best_conn_to_exit_router(s, bf, msg, lp, group_list);
                /*if(my_router_id == 5){
                    tw_output(lp,"\n Chosen intm_grp: %d", msg->intm_grp_id); //KBDEBUG
                    tw_output(lp,"\n next_hop rotuer: lid: %d  gid: %d\n", best_conn.dest_lid, best_conn.dest_gid); //KBDEBUG
                }*/
                conns.push_back(best_conn);
                //tw_output(lp,"Returned best conn. msg->src_terminal_id: %d\n", msg->src_terminal_id); //KBDEBUG
                return conns;  // Return best connection based on UGAL 
            }
            //if(my_router_id == 5 &&  msg->packet_ID > 205)
            //    tw_error(TW_LOC, "Done DEBUGGING Router.\n"); //KBDEBUG
/*
...
/*
            if(strcmp(global_routing_algo, "ugal_g_plus") == 0 && my_group_id != fdest_group_id) {
                for(vector< int >:: iterator it = group_list.begin() ; it != group_list.end() ; it++){
                    if(my_router_id == 0)
                    {
                        tw_output(lp,"Selected group: %d \n", *it); //KBDEBUG
                        tw_output(lp,"   packet_ID      : %d \n", msg->packet_ID);
                        tw_output(lp,"   src_terminal_id: %d \n", msg->src_terminal_id);
                        tw_output(lp,"   dst_terminal_id: %x \n", msg->dfdally_src_terminal_id);
                        tw_output(lp,"   my_router_id   : %d \n", my_router_id);
                        tw_output(lp,"   my_group_id    : %d \n", my_group_id);
                        tw_output(lp,"   origin_group_id: %d \n", origin_group_id);
                        tw_output(lp,"   fdest_router_id: %d \n", fdest_router_id);
                        tw_output(lp,"   fdest_group_id : %d \n", fdest_group_id);
                        tw_output(lp," get_router_gids_with_global_to_group(%d): ", *it); //KBDEBUG
                        vector< int > temp_conns1 = s->connMan.get_router_gids_with_global_to_group(*it);
                        for(vector<int>:: iterator it1 = temp_conns1.begin() ; it1 != temp_conns1.end() ; it1++)
                        {
                            tw_output(lp,"  router_gid  : %d\n", *it1);
                        }
                        vector< Connection > temp_conns2 = s->connMan.get_connections_to_group(*it);
                        tw_output(lp," get_connections_to_group(%d):\n", *it); //KBDEBUG
                        for (vector<Connection>::iterator it2 = temp_conns2.begin(); it2 != temp_conns2.end(); it2++) 
                        {

                            Connection conn = *it2;
                            tw_output(lp,"  router_gid  : %d\n", conn.port);
                            tw_output(lp,"  src_lid     : %d\n", conn.src_lid);
                            tw_output(lp,"  src_gid     : %d\n", conn.src_gid);
                            tw_output(lp,"  src_group_id: %d\n", conn.src_group_id);
                            tw_output(lp,"  dest_lid    : %d\n", conn.dest_lid);
                            tw_output(lp,"  dest_gid    : %d\n", conn.dest_gid);
                            tw_output(lp,"  dst_group_id: %d\n", conn.dest_group_id);
                            if ( conn.conn_type == CONN_GLOBAL) tw_output(lp,"  GLOBAL\n\n"); else tw_output(lp,"  OTHER-CONN-TYPE\n\n");
                        }
                        tw_output(lp,"\n   ============================= \n");
                    }
                    // get router with conn to selected group;

                    // get conn on router to the intermediate group
                }
...
*/
            ...
            if(strcmp(global_routing_algo, "ugal_g_plus") == 0 && my_group_id != fdest_group_id) {
                //tw_output(lp,"\nPacketID: %d", msg->packet_ID); //KBDEBUG
                vector< Connection > tmp_conns = s->connMan.get_connections_to_group(msg->ugal_intm_grp_id);
                return tmp_conns; // KBDEBUG - not valid when there are multiple connection to the intm group
...
static Connection dfdally_prog_adaptive_routing(router_state *s, tw_bf *bf, terminal_dally_message *msg, tw_lp *lp, int fdest_router_id)
{
    ...
    // This was extracted from do_dfdally_routing and modified. Potentially, it could be re-integrated there to allow other routing to use local nonmin hops.
    // If source group != dest group, this does not allow local routing in source group.
    //
    // // Local Destination Group Routing --------------
    if (my_router_id == fdest_router_id) { //destination router reached, next dest = final terminal destination
        vector< Connection > poss_next_stops = s->connMan.get_connections_to_gid(msg->dfdally_dest_terminal_id, CONN_TERMINAL);
        ...
        // Find indirect (nonmin) connections to dest router
        // TODO: this doesn't work with failed intra-group links (Kevin, Oct 2022.10.10)
        if((msg->last_hop == GLOBAL && prev_vcg_chan < 2) || msg->last_hop == TERMINAL)
        { // if we are starting in the dst group and have enough free VCs to take a local non-min hop. Packets from terminals are in VC0
            ...
            // select intermediate router 2 intermediate routers at random
            ...
            msg->num_rngs++;
            msg->num_rngs++;
            int rand_sel1 = tw_rand_integer(lp->rng, 0, l_intm_router_list.size()-1);
            int rand_sel2 = tw_rand_integer(lp->rng, 0, l_intm_router_list.size()-1);
            if (rand_sel1 == rand_sel2)
                rand_sel2 = (rand_sel2  + 1) % l_intm_router_list.size();

            int l_intm_router_id1 = l_intm_router_list[rand_sel1];
            int l_intm_router_id2 = l_intm_router_list[rand_sel2];
            l_intm_router_id1 = l_intm_router_id1 + my_group_id*num_routers;
            l_intm_router_id2 = l_intm_router_id2 + my_group_id*num_routers;

            // record connections to intermediate routers
            vector< Connection > tmp_conns;
            tmp_conns = s->connMan.get_connections_to_gid(l_intm_router_id1, CONN_LOCAL);
            for (vector<Connection>::iterator it = tmp_conns.begin(); it != tmp_conns.end(); it ++) {
                nmin_conns_to_fdest.insert(nmin_conns_to_fdest.begin(), *it);
            }
            tmp_conns = s->connMan.get_connections_to_gid(l_intm_router_id2, CONN_LOCAL);
            for (vector<Connection>::iterator it = tmp_conns.begin(); it != tmp_conns.end(); it ++) {
                nmin_conns_to_fdest.insert(nmin_conns_to_fdest.begin(), *it);
            }
        }
        // find direct connections to dest router
        ...
        min_score = dfdally_apply_advanced_scoring(s, bf, msg, best_min_conn.port, lp, C_MIN, min_score, false, 2);
        nonmin_score = dfdally_apply_advanced_scoring(s, bf, msg, best_nonmin_conn.port, lp, C_NONMIN, nonmin_score, false, 2);
        ...
        if (upper_threshold_exceeded){ // log decision due to upper threshold
            if (min_score <= nonmin_score) {
                s->route_upper_threshold_for_min_score[best_min_conn.port][vcg]++;
            } else {
                s->route_upper_threshold_for_nonmin_score[best_min_conn.port][vcg]++;
            }
        } else { // log decision due to scores or lower threshold
            if (min_score <= nonmin_score) {
                s->route_min_score[best_min_conn.port][vcg]++;
                if (min_score > adaptive_threshold && !upper_threshold_exceeded)
                    s->route_min_score_only[best_min_conn.port][vcg]++;
            } else {
                if (min_score <= adaptive_threshold){
                    s->route_lower_threshold[best_min_conn.port][vcg]++;
                } else {
                        s->route_nonmin_score[best_nonmin_conn.port][vcg]++;
                }
            }
        }
        #endif
        ...
        if (min_score <= adaptive_threshold ||
            ...
            if(SCORED_ROUTER(s->router_id)) {
		// get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
		fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), 2, 8, best_min_conn.port, -1); // 8 for min connection type
	    }
	    #endif
            ...
            if(SCORED_ROUTER(s->router_id)) {
		// get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
		fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), 2, 9, best_nonmin_conn.port, -1); // 9 for non-min connection type
	    }
	    #endif
    ...
    if ((my_group_id != origin_group_id) && (my_group_id != fdest_group_id)){
        // if we're moving to a second intermediate group, raise an error
        if (msg->is_intm_visited == 1)
            assert(msg->intm_grp_id == my_group_id);
        else{
            if( msg->packet_ID >= 201 && msg->packet_ID < 205 && msg->src_terminal_id == 27){
                tw_output(lp,"\n ===== Original intm_grp: %d Chosen intm_grp: %d\n", msg->intm_grp_id, my_group_id); //KBDEBUG
                if( msg->packet_ID > 205 && msg->src_terminal_id == 27)
                    tw_error(TW_LOC, "Done DEBUGGING Router.\n"); //KBDEBUG
            }
            msg->intm_grp_id = my_group_id; // KBEDIT
            msg->is_intm_visited = 1;
    ...
    if(SCORED_ROUTER(s->router_id)) {
	int packet_group = 0;
	if(s->group_id == msg->origin_router_id / s->params->num_routers)
	    packet_group = 0;
	else
	    packet_group = 1;

        // get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
        fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d ", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 0); // 0 for min connection type
    }
    #endif
	...
	if(s->group_id == msg->origin_router_id / s->params->num_routers)
        ...
        // get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
        fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d ", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 1); // 1 for non-min connection type
    }
    #endif
	...
	if(s->group_id == msg->origin_router_id / s->params->num_routers)
        ...
        if (min_score != INT_MAX)
            fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 2, best_min_conn.port, min_score); // 2 for best min score 
        if (nonmin_score != INT_MAX)
            fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 3, best_nonmin_conn.port, nonmin_score); // 3 for best nonmin score
    }
    #endif

    if ((msg->path_type == NON_MINIMAL) && (msg->is_intm_visited != 1)) { //if we're nonminimal and haven't reached the intermediate group yet
        assert(s->group_id == msg->origin_router_id / num_routers); // Verify that we're in the source group
        #if DEBUG_ROUTING_DECISION == 1
        int vcg = 0;
        if (s->params->num_qos_levels > 1)
            vcg = get_vcg_from_category(msg);
		...
		// get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
		fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 10, best_nonmin_conn.port, -1); // 10 for non-min connection type
	    }
	    #endif
    ...
    if ((my_group_id != origin_group_id) && (my_group_id != fdest_group_id)) {
        ...
        // confirm that we're in the source group
        assert(my_group_id == msg->origin_router_id / s->params->num_routers);
    }
    assert(group_code == 0 || group_code == 1); // may be redunant

    min_score = dfdally_apply_advanced_scoring(s, bf, msg, best_min_conn.port, lp, C_MIN, min_score, is_global_adaptive, group_code);
    nonmin_score = dfdally_apply_advanced_scoring(s, bf, msg, best_nonmin_conn.port, lp, C_NONMIN, nonmin_score, is_global_adaptive, group_code);

    bool upper_threshold_exceeded = false;
    //if (group_code == 0 && exceed_adaptive_upper_threshold(s, bf, msg, best_min_conn, C_MIN) == true &&
    if (group_code == 0){
        if (exceed_adaptive_upper_threshold(s, bf, msg, best_min_conn, C_MIN, fdest_group_id) == true &&
    ...
    if (min_score <= adaptive_threshold ||
		    ...
		    if (s->group_id == msg->origin_router_id / s->params->num_routers)
			   packet_group = 0;
			    ...
			    packet_group = 1;
		// get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
		fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 6, best_min_conn.port, -1); // 6 for min connection type
	    }
	    #endif
        ...
        msg->path_type = NON_MINIMAL;
		    ...
		    if ( s->group_id == msg->origin_router_id / s->params->num_routers )
			   packet_group = 0;
			    ...
			    packet_group = 1;
		// get_vcg_from_category(msg) may cause an error if category isn't a known QoS level
		fprintf(dragonfly_route_score_log, "\n %.0f %d %d %d %d %d:%d", tw_now(lp), s->router_id, get_vcg_from_category(msg), packet_group, 7, best_nonmin_conn.port, -1); // 7 for non-min connection type
	    }
	    #endif
